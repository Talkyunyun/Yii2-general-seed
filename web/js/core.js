$(function() {
    $('.openWin').click(function() {
        ...
        self.parent.APP.openPage(url, title, function() {
        ...
        $.ajax({
            ...
            success : function(res) {
                ...
                if (res.code == 0) {
                    ...
                    self.parent.APP.showMsg(res.msg, 'error');
                    callback(false);
                }
            },
            beforeSend : function() {
            ...
            error: function() {
                layer.close(loading);
                self.parent.APP.showMsg('网络请求失败!', 'error');
                callback(false);
            }
        });
    });
...
// 时间选择插件
Core.prototype.showDate = function(th) {
...
Core.prototype.uploadImg = function(param, callback) {
    ...
    layui.use('layer', function() {
        ...
        uploader.on('uploadError', function(file, res) {
            self.parent.APP.showMsg('网络请求错误', 'error');
            callback(false);
        });
        // 文件上传完成,不管成功还是失败都会执行
...
Core.prototype.openParentWin = function(param) {
    ...
    APP.parent.APP.openPage(newParam);
}
